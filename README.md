...
`MockDataSource` имитирует и задержку сети (~350 мс с джиттером), а не только
форму данных. Без неё `Promise.resolve()` выполнялся бы в том же тике, `isLoading`
жил бы один кадр, и состояния загрузки стали бы недостижимыми — а значит и
непроверяемыми.
...
- ✅ Этап 5 — ленивая карта, тема, адаптив, PWA, состояния загрузки/ошибки
- ⏳ Этап 6 — тесты компонентов и документация (следующий)
