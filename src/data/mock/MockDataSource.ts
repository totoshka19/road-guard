...
/**
 * Имитация задержки сети для «REST»-запросов.
 *
 * Без неё Promise.resolve() выполняется в том же тике, isLoading живёт один
 * кадр, и состояния загрузки становятся недостижимыми — а значит и
 * непроверяемыми. Настоящий бэкенд отвечает не мгновенно, и мок должен быть
 * честным не только по форме данных.
 */
const LATENCY_MS = 350
const LATENCY_JITTER = 0.4

function withLatency<T>(value: T): Promise<T> {
  const ms = LATENCY_MS * (1 + (Math.random() * 2 - 1) * LATENCY_JITTER)
...
export class MockDataSource implements DataSource {
  ...
  getCameras(): Promise<Camera[]> {
    return withLatency(this.cameras)
  }

  getViolations(filters: ViolationFilters = {}): Promise<Violation[]> {
    return withLatency(this.history.filter((v) => matchesFilters(v, filters)))
  }
